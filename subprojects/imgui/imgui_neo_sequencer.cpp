...
static ImGuiNeoSequencerStyle& neoStyle()
{
	static ImGuiNeoSequencerStyle st;
	return st;
}
...
static ImRect getCurrentFrameBB(uint32_t frame, ImGuiNeoSequencerInternalData& context)
{
	...
	const auto width = neoStyle().CurrentFramePointerSize * GetIO().FontGlobalScale;
	const auto cursor =
	...
	float pointerHeight = neoStyle().CurrentFramePointerSize * 2.5f;
...
static void renderCurrentFrame(ImGuiNeoSequencerInternalData& context)
{
	...
	if (isFrameInTimeline(context, context.FinalCurrentFrame))
	{
		RenderNeoSequencerCurrentFrame(GetStyleNeoSequencerColorVec4(ImGuiNeoSequencerCol_FramePointerLine),
									   ...
									   neoStyle().CurrentFrameLineWidth, drawList);
	}
}

static void processAndRenderZoom(ImGuiNeoSequencerInternalData& context,
								 bool allowEditingLength,
								 uint32_t* start,
								 uint32_t* end)
{
	const auto& imStyle = GetStyle();
	...
	const auto zoomHeight = GetFontSize() * neoStyle().ZoomHeightScale;
...
bool BeginNeoSequencer(const char* idin,
					   uint32_t* frame,
					   uint32_t* startFrame,
					   uint32_t* endFrame,
					   const ImVec2& size,
					   ImGuiNeoSequencerFlags flags)
{
	...
	context.TopBarStartCursor =
		showZoom ? cursor + ImVec2{0, GetFontSize() * neoStyle().ZoomHeightScale + imStyle.FramePadding.y} : cursor;
	...
	RenderNeoSequencerBackground(GetStyleNeoSequencerColorVec4(ImGuiNeoSequencerCol_Bg), context.StartCursor,
								 context.Size, drawList, neoStyle().SequencerRounding);
	...
	RenderNeoSequencerTopBarOverlay(context.Zoom, context.ValuesWidth, context.StartFrame, context.EndFrame,
									...
									neoStyle().TopBarShowFrameLines, neoStyle().TopBarShowFrameTexts);

	context.TopBarSize = ImVec2(context.Size.x, neoStyle().TopBarHeight);
	...
	context.FilledHeight = context.TopBarSize.y + neoStyle().TopBarSpacing +
						   (showZoom ? imStyle.FramePadding.y + neoStyle().ZoomHeightScale + GetFontSize() : 0.0f);

	context.StartValuesCursor = context.TopBarStartCursor + ImVec2{0, context.TopBarSize.y + neoStyle().TopBarSpacing};
...
bool BeginNeoTimeline(const char* label, bool* open, ImGuiNeoTimelineFlags flags)
{
	...
	if (addRes)
	{
		...
		RenderNeoTimelineLabel(label,
							   context.ValuesCursor + imStyle.FramePadding +
								   ImVec2{(float) currentTimelineDepth * neoStyle().DepthItemSpacing, 0},
							   labelSize, color, isGroup, isGroup && (*open));
	}

	if (is_group.back())
	{
		context.ValuesCursor.x += imStyle.FramePadding.x + (float) currentTimelineDepth * neoStyle().DepthItemSpacing;
...
void PushNeoSequencerStyleColor(ImGuiNeoSequencerCol idx, ImU32 col)
{
	...
	backup.Col = idx;
	backup.BackupValue = neoStyle().Colors[idx];
	sequencerColorStack.push_back(backup);
	neoStyle().Colors[idx] = ColorConvertU32ToFloat4(col);
}
...
void PopNeoSequencerStyleColor(int count)
{
	while (count > 0)
	{
		...
		neoStyle().Colors[backup.Col] = backup.BackupValue;
...
ImGuiNeoSequencerStyle::ImGuiNeoSequencerStyle()
{
	...
	ImVec4* neoColors = Colors;

	// 배경 계열
	neoColors[ImGuiNeoSequencerCol_Bg] = style.Colors[ImGuiCol_WindowBg];
	neoColors[ImGuiNeoSequencerCol_TopBarBg] = style.Colors[ImGuiCol_Header];
	neoColors[ImGuiNeoSequencerCol_TimelinesBg] = style.Colors[ImGuiCol_Header];
	neoColors[ImGuiNeoSequencerCol_TimelineBorder] = style.Colors[ImGuiCol_Border];

	// 타임라인 선택 강조
	neoColors[ImGuiNeoSequencerCol_SelectedTimeline] = style.Colors[ImGuiCol_HeaderActive];

	// 프레임 포인터
	neoColors[ImGuiNeoSequencerCol_FramePointer] = style.Colors[ImGuiCol_SliderGrab];
	neoColors[ImGuiNeoSequencerCol_FramePointerHovered] = style.Colors[ImGuiCol_SliderGrabActive];
	neoColors[ImGuiNeoSequencerCol_FramePointerPressed] = style.Colors[ImGuiCol_ButtonActive];
	neoColors[ImGuiNeoSequencerCol_FramePointerLine] = style.Colors[ImGuiCol_SeparatorActive];

	// 키프레임
	neoColors[ImGuiNeoSequencerCol_Keyframe] = style.Colors[ImGuiCol_Button];
	neoColors[ImGuiNeoSequencerCol_KeyframeHovered] = style.Colors[ImGuiCol_ButtonHovered];
	neoColors[ImGuiNeoSequencerCol_KeyframePressed] = style.Colors[ImGuiCol_ButtonActive];
	neoColors[ImGuiNeoSequencerCol_SelectedKeyframe] = style.Colors[ImGuiCol_PlotLinesHovered];
	neoColors[ImGuiNeoSequencerCol_KeyframeWithCurrentFrame] = style.Colors[ImGuiCol_PlotHistogramHovered];

	// 줌바
	neoColors[ImGuiNeoSequencerCol_ZoomBarBg] = style.Colors[ImGuiCol_FrameBg];
	neoColors[ImGuiNeoSequencerCol_ZoomBarSlider] = style.Colors[ImGuiCol_SliderGrab];
	neoColors[ImGuiNeoSequencerCol_ZoomBarSliderHovered] = style.Colors[ImGuiCol_SliderGrabActive];
	neoColors[ImGuiNeoSequencerCol_ZoomBarSliderEnds] = style.Colors[ImGuiCol_FrameBg];
	neoColors[ImGuiNeoSequencerCol_ZoomBarSliderEndsHovered] = style.Colors[ImGuiCol_FrameBgHovered];
}
